...
/**
 * Page title for build landing pages.
 *   nickname present: "Nickname" or "Nickname with Drop Fixes"
 *   nickname absent:  "officialName" or "officialName with Drop Fixes"
 */
export function buildPageTitle(entry: BuildEntry): string {
  ...
  if (entry.nickname) {
    ...
    // Keep dated names lowercase; capitalize word-only names.
    base = /\d/.test(raw) ? raw : raw.charAt(0).toUpperCase() + raw.slice(1);
  }
  return entry.fixed ? `${base} with Drop Fixes` : base;
}

/** Subtitle with the original build name and revision. */
